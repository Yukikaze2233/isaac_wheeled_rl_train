...
更新：2026-09-14。基础分支：`isaac60`；A训练使用冻结版本`6ed83408d1d1cb02ea687e77fa80e5c20932f36c`。用户要求增加轮次后，总预算细化为**100000次更新**；目前实际执行的仍是A的10000次，B–E属于待实现、待验证的课程。
...
- 首先围绕已读回的名义有效μ=0.5，训练effective μ∈[0.4,0.6]；通过后候选扩到[0.3,0.8]。必须记录两端材质、合成模式和实际shape读回，静摩擦不低于动摩擦。
...
- 至少保留30%名义物理参数的回合。先逐项对照，再混合；启动配置含DR字段不等于DR已经作用到物理状态或`compute_torques`。
